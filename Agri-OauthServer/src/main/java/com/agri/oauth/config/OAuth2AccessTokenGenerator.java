...
public class OAuth2AccessTokenGenerator {
	 ...
	 */
	public OAuth2AccessToken generate(@NonNull AuthUserDetail userDetails, @NonNull String clientId) throws ClientRegistrationException, AuthenticationException {
		 TokenRequest tokenRequest = new TokenRequest(new HashMap<>(), clientId, Collections.singletonList(SCOPE), PASSWORD_GRANT_TYPE);
		ClientDetails client = clientDetailsService.loadClientByClientId(clientId);
		 UsernamePasswordAuthenticationToken authToken = new UsernamePasswordAuthenticationToken(userDetails, userDetails, userDetails.getAuthorities());
		 OAuth2Authentication authentication = new OAuth2Authentication(tokenRequest.createOAuth2Request(client), authToken);
